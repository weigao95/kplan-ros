...
from kplan.keypoint_grasp.shoe_grasp_planner import ShoeUprightGraspPlanner
from kplan_ros.srv import KeypointGraspingService, KeypointGraspingServiceRequest, KeypointGraspingServiceResponse


class ShoeKeypointGraspPlanningServer(object):

    def __init__(self):
        self._planner = ShoeUprightGraspPlanner()
    ...
    def handle_grasp_planning_request(self, request):
        ...
        if n_cloud_point > 0:
            ...
            # Some fake input...
            pc_in_world = np.zeros(shape=(1, 3))
...
def main():
    server = ShoeKeypointGraspPlanningServer()
    server.run()
